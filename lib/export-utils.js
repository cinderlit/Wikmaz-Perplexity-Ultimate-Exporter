(function (root) {
  ...
  function buildFilename(chatData) {
    ...
    return "Perplexity_Export/" + safeSpace + "/" + safeTitle + " (" + safeUuid + ").md";
  }

  function buildManifestFilename(runTimestamp) {
    const safeTs = (runTimestamp || new Date().toISOString()).replace(/[:.]/g, "-");
    return "Perplexity_Export/manifest-" + safeTs + ".json";
  }
